...
==Tip:== We can also combine the `customIssuePrefixesAlign` configuration item to dynamically change the position of the issue prefix option.
...
::: tip
If `cz-git` detects that `allowCustomIssuePrefixes` and `allowEmptyIssuePrefixes` have very strict rules (both set to false) and the **issuePrefixes selection list has only one item**, it will automatically skip question and output
:::
