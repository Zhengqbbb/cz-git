...
export class SearchCheckbox extends Base {
    ...
    // Render question
        ...
        if (this.status === 'answered') {
            content += this.themeColorCode
                ? style.rgb(this.themeColorCode)(this.selection.join(this.separator))
                : style.cyan(this.selection.join(this.separator))
        }
 ...
 */
function choicesRender(choices: ChoicesType['choices'], pointer: number, themeColorCode?: string): string {
    let output = ''
    ...
    choices.forEach((choice, i) => {
        ...
        if (choice.disabled) {
            ...
            if (i - separatorOffset === pointer) {
                output += themeColorCode
                    ? style.rgb(themeColorCode)(` ${figures.pointer}${line}`)
                    : style.cyan(` ${figures.pointer}${line}`)
            }
            else {
                output += `  ${line}`
            }
