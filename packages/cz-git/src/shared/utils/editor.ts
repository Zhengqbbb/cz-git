...
import fs, { constants as cnst } from 'node:fs'
...
function tempOpen(affixes: string | AffixOptions | undefined, callback: (err: any, result: OpenFile) => void) {
...
export function editCommit(answers: Answers, options: CommitizenGitOptions, cb: (message: string) => void) {
