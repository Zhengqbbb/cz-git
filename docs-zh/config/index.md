...
module.exports = {
  ...
  prompt: {
    messages: {
      ...
      customFooterPrefixes: "输入自定义issue前缀 :",
...
module.exports = {
  ...
  prompt: {
    ...
    customIssuePrefixesAlign: "top",
    emptyIssuePrefixesAlias: "跳过",
    customIssuePrefixesAlias: "自定义前缀",
    allowCustomIssuePrefixes: true,
    allowEmptyIssuePrefixes: true,
    confirmColorize: true,
    maxHeaderLength: Infinity,
...
::: tip
推荐在项目中使用JavaScript进行配置文件，你可以结合`fs`和`path` 为生成动态模块选择[范围](/recipes/#scopes)
:::
